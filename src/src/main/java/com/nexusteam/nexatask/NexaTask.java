...
public abstract class NexaTask<Params, Progress, Result> {
     ...
     */
    @NonNull
    public final NexaTask<Params, Progress, Result> executeOnExecutor(@NonNull ExecutorService executor,
                                                                       @Nullable Params... params) {
        if (!mState.compareAndSet(TaskState.PENDING, TaskState.RUNNING)) {
            throw new IllegalStateException("Cannot execute task: current state = " + mState.get());
        }
        
        mStartTime = System.currentTimeMillis();
        
        if (mLoggingEnabled) {
            TaskLogger.d(LIBRARY_NAME, "Task started: " + mTaskId);
        }
        
        // Run on UI thread first
    ...
    public final boolean cancel() {
        if (mState.get() == TaskState.RUNNING && !mCancelled.get()) {
            ...
            if (mState.compareAndSet(TaskState.RUNNING, TaskState.CANCELLED)) {
                ...
                if (mLoggingEnabled) {
                    TaskLogger.d(LIBRARY_NAME, "Task cancelled: " + mTaskId);
                }
                return true;
            }
        }
    ...
    public final boolean pause() {
        if (mState.get() == TaskState.RUNNING && !mIsPaused.get()) {
            mIsPaused.set(true);
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
            ...
            if (mLoggingEnabled) {
                TaskLogger.d(LIBRARY_NAME, "Task paused: " + mTaskId);
            }
            return true;
        }
        return false;
    ...
    public final boolean resume() {
        if (mIsPaused.get()) {
            ...
            if (mLoggingEnabled) {
                TaskLogger.d(LIBRARY_NAME, "Task resumed: " + mTaskId);
            }
            return true;
        }
        return false;
    ...
    protected final void publishProgress(@Nullable final Progress... values) {
        if (!mCancelled.get() && mState.get() == TaskState.RUNNING && values != null && values.length > 0) {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    try {
                        ...
                        if (mLoggingEnabled) {
                            TaskLogger.w(LIBRARY_NAME, "Progress update error", e);
                        }
                    }
                }
            });
    ...
    public final Status getStatus() {
        TaskState state = mState.get();
        switch (state) {
            case PENDING: return Status.PENDING;
            case RUNNING: return mIsPaused.get() ? Status.PAUSED : Status.RUNNING;
